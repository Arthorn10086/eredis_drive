...
-define(CRC16Def, [16#0000, 16#1021, 16#2042, 16#3063, 16#4084, 16#50a5, 16#60c6, 16#70e7,
...
calcByte(Fcs, B) ->
    ...
    ((Fcs bsl 8) band 16#ffff) bxor lists:nth(Idx + 1, ?CRC16Def).

crc16(X) when is_list(X) ->
    lists:foldl(fun(B, Fsc) -> calcByte(Fsc, B) end, 16#0, X).